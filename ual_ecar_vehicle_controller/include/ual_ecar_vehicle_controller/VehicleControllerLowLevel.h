...
class VehicleControllerLowLevel : public COutputLogger
{
   ...
   protected:
	...
	bool AttemptConnection(CSerialPort& m_serial);  //!< Returns true if
													//!< connected OK, false on
													//!< error.
	bool IsConnected() const;  //!< Returns true if the serial comms are working
	bool ReceiveFrameFromController(
		std::vector<uint8_t>& rx_data,
		CSerialPort& m_serial);  //!< Tries to get a framed chunk of data from
								 //!< the controller.
	void processIncommingFrame(
	...
	bool WriteBinaryFrame(
		...
		CSerialPort& m_serial);  //!< Sends a binary packet, in the
								 //!< expected format  (returns false on
								 //!< COMMS error)
	bool SendFrameAndWaitAnswer(
		const uint8_t* full_frame, const size_t full_frame_len,
		CSerialPort& m_serial, const int num_retries = 10,
