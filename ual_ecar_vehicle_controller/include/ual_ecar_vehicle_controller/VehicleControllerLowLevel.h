...
protected:
  ...
  void
  daqOnNewControlSignalCallback(const TFrame_CONTROL_SIGNAL_payload_t &data);
  ...
  bool ReceiveFrameFromController(
      std::vector<uint8_t> &rx_data); //!< Tries to get a framed chunk of data
                                      //!< from the controller.
  void processIncommingFrame(const std::vector<uint8_t> &rxFrame);
  bool WriteBinaryFrame(
      const uint8_t *full_frame,
      const size_t full_frame_len); //!< Sends a binary packet, in the expected
                                    //!< format  (returns false on COMMS error)
  bool SendFrameAndWaitAnswer(
      ...
      uint8_t expected_ans_opcode =
          0 //<! 0 means the default convention: full_frame[1]+0x70,
