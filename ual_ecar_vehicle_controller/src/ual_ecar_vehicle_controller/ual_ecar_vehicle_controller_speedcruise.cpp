...
bool VehicleControllerLowLevel::initialize() {
	...
	m_serial_SpeedCruise_port_name = "/dev/serial/by-id/usb-UAL_Claraquino_#1__FT232R_USB_UART__A71VGDJN-if00-port0";
	m_serial_SpeedCruise_port_baudrate = 500000;
	m_nh_params.getParam("SERIAL_PORT", m_serial_SpeedCruise_port_name);
	m_nh_params.getParam("SERIAL_PORT_BAUDRATE", m_serial_SpeedCruise_port_baudrate);
...
void VehicleControllerLowLevel::processIncommingFrame(const std::vector<uint8_t> &rxFrame, CSerialPort &m_serial, std::string &m_serial_port_name) {
	...
	if (m_serial_port_name == m_serial_SpeedCruise_port_name)
	{
	if (rxFrame.size() >= 5) {
		switch (rxFrame[1]) {
...
bool VehicleControllerLowLevel::iterate() {
	...
	if (m_modes_changed) {
		...
		WriteBinaryFrame(reinterpret_cast<uint8_t *>(&cmd), sizeof(cmd)); // Add m_serial
	...
	if (!m_autonomous_driving_mode && m_joy_changed) {
		...
		if (m_mode_openloop_throttle) {
			...
			cmd.calc_and_update_checksum();
			WriteBinaryFrame(reinterpret_cast<uint8_t *>(&cmd), sizeof(cmd)); //Add m_serial_SpeedCruise
...
// ---
	...
	while (ReceiveFrameFromController(rxFrame, m_serial_SpeedCruise) && ++nFrames < MAX_FRAMES_PER_ITERATE) {
		// Process them:
		processIncommingFrame(rxFrame,m_serial_SpeedCruise,m_serial_SpeedCruise_port_name); // dupl.
	}
	...
	if (!nFrames && m_NOP_sent_counter++ > 20) {
		...
		TFrameCMD_NOP cmd;
		cmd.calc_and_update_checksum();
		return WriteBinaryFrame(reinterpret_cast<uint8_t *>(&cmd), sizeof(cmd)); // Add Serial
	}
// ---
...
bool VehicleControllerLowLevel::AttemptConnection() {
	...
	try {
		m_serial_SpeedCruise.open(m_serial_SpeedCruise_port_name);

		// Set basic params:
		m_serial_SpeedCruise.setConfig(m_serial_SpeedCruise_port_baudrate);
		m_serial_SpeedCruise.setTimeouts(100, 0, 10, 0, 50);

		MRPT_LOG_INFO_FMT("[VehicleControllerLowLevel::AttemptConnection] Serial port '%s' open was successful.",m_serial_SpeedCruise_port_name.c_str());
		return true;
...
bool VehicleControllerLowLevel::SendFrameAndWaitAnswer(const uint8_t *full_frame, const size_t full_frame_len, const int num_retries, const int retries_interval_ms, uint8_t expected_ans_opcode) {
	...
	for (int iter = 0; iter < num_retries; iter++) {
		...
		// Send:
		if (!WriteBinaryFrame(full_frame, full_frame_len, m_serial_SpeedCruise))
			continue;

		// Wait for answer:
		std::vector<uint8_t> rxFrame;
		if (this->ReceiveFrameFromController(rxFrame,m_serial_SpeedCruise) && rxFrame.size() > 4) {
			...
			if (RX_OPCODE == expected_ans_opcode) {
				...
				processIncommingFrame(rxFrame, m_serial_SpeedCruise,m_serial_SpeedCruise_port_name);
			}
		}
	}
