...
bool VehicleControllerLowLevel::initialize() {
  ...
  // Publisher: ADC data
  m_pub_ADC = m_nh.advertise<ual_ecar_vehicle_controller::AnalogReading>(
      "claraquino_adc", 10);

  // Publisher: ENC data
  m_pub_ENC = m_nh.advertise<ual_ecar_vehicle_controller::EncodersReading>(
      "claraquino_encoders", 10);
  ...
  m_pub_Control_signal =
      m_nh.advertise<ual_ecar_vehicle_controller::ControlSignal>(
          "claraquino_control_signal", 10);

  m_sub_contr_status[0] =
      m_nh.subscribe("vehicle_openloop_mode_steering", 10,
                     &VehicleControllerLowLevel::modeSteeringCallback, this);
  m_sub_contr_status[1] =
      m_nh.subscribe("vehicle_openloop_mode_throttle", 10,
                     &VehicleControllerLowLevel::modeThrottleCallback, this);
  m_sub_eje_x = m_nh.subscribe("joystick_eje_x", 10,
                               &VehicleControllerLowLevel::ejexCallback, this);
  m_sub_eje_y = m_nh.subscribe("joystick_eje_y", 10,
                               &VehicleControllerLowLevel::ejeyCallback, this);
  m_sub_autonomous_driving =
      m_nh.subscribe("vehicle_autonomous_mode", 10,
...
bool VehicleControllerLowLevel::iterate() {
  ...
  while (ReceiveFrameFromController(rxFrame) &&
...
void VehicleControllerLowLevel::daqOnNewADCCallback(
    const TFrame_ADC_readings_payload_t &data) {
...
void VehicleControllerLowLevel::daqOnNewENCCallback(
    const TFrame_ENCODERS_readings_payload_t &data) {
...
void VehicleControllerLowLevel::daqOnNewControlSignalCallback(
    const TFrame_CONTROL_SIGNAL_payload_t &data) {
...
bool VehicleControllerLowLevel::AttemptConnection() {
  ...
  try {
    ...
    MRPT_LOG_INFO_FMT("[VehicleControllerLowLevel::AttemptConnection] Serial "
                      "port '%s' open was successful.",
...
bool VehicleControllerLowLevel::WriteBinaryFrame(const uint8_t *full_frame,
...
bool VehicleControllerLowLevel::SendFrameAndWaitAnswer(
    ...
    const int num_retries, const int retries_interval_ms,
  ...
  for (int iter = 0; iter < num_retries; iter++) {
    ...
    if (this->ReceiveFrameFromController(rxFrame) && rxFrame.size() > 4) {
      ...
      if (RX_OPCODE == expected_ans_opcode) {
        ...
        MRPT_LOG_INFO_FMT("SendFrameAndWaitAnswer(): Rx ACK for OPCODE=0x%02X "
                          "after %i retries.",
  ...
  MRPT_LOG_ERROR_FMT("SendFrameAndWaitAnswer(): Missed ACK for OPCODE=0x%02X",
  ...
  */
  ...
  while (nFrameBytes < (lengthField = (1 + 1 + 1 + buf[2] + 1 + 1))) {
    ...
    try {
      ...
      // Disconnected?
      MRPT_LOG_ERROR_FMT("ReceiveFrameFromController(): Comms error: %s",
                         e.what());
      return false;
    }
