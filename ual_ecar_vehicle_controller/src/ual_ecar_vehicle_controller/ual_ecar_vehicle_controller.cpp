...
bool VehicleControllerLowLevel::initialize() {
	...
	m_serial_SpeedCruise_port_name = "/dev/serial/by-id/usb-UAL_Claraquino_#1__FT232R_USB_UART__A71VGDJN-if00-port0"; // CHANGE
	...
	if (this->AttemptConnection(m_serial_Steer, m_serial_Steer_port_name, m_serial_Steer_port_baudrate)) {
	...
	if (this->AttemptConnection(m_serial_SpeedCruise, m_serial_SpeedCruise_port_name, m_serial_SpeedCruise_port_baudrate)) {
  ...
  // Subscriber:
	...
	*/
	// Decimation params
	{
		TFrameCMD_VERBOSITY_CONTROL_payload_t Decimation_config_Steer, Decimation_config_SpeedCruise;
		int decimate_ADC_Steer=10, decimate_ENCABS_Steer=10, decimate_CPU_Steer=10000, decimate_CONTROLSIGNAL_Steer = 10, decimate_ENCINC_Steer = 10;
		...
		m_nh_params.getParam("DECIM_CONTROLSIGNAL_STEER",decimate_CONTROLSIGNAL_Steer);

		if (decimate_ADC_Steer>0 && decimate_ENCABS_Steer>0 && decimate_CPU_Steer>0 && decimate_CONTROLSIGNAL_Steer>0 && decimate_ENCINC_Steer>0)
		{
			...
			Decimation_config_Steer.decimate_CONTROLSIGNAL = decimate_CONTROLSIGNAL_Steer;

			MRPT_LOG_INFO_FMT(" Steer Decimation: ADC=%i  ENCABS=%i  ENCINC=%i  CPU=%i  Control Signal=%i",decimate_ADC_Steer, decimate_ENCABS_Steer, decimate_ENCINC_Steer, decimate_CPU_Steer, decimate_CONTROLSIGNAL_Steer);
			this->CMD_Decimation_configuration(Decimation_config_Steer, m_serial_Steer);
		}

		int decimate_ADC_SpeedCruise=10, decimate_CPU_SpeedCruise=10000, decimate_CONTROLSIGNAL_SpeedCruise = 10, decimate_ENCINC_SpeedCruise = 10;

		m_nh_params.getParam("DECIM_ADC_SPEEDCRUISE", decimate_ADC_SpeedCruise);
		m_nh_params.getParam("DECIM_ENCINC_SPEEDCRUISE",decimate_ENCINC_SpeedCruise);
		m_nh_params.getParam("DECIM_CPU_SPEEDCRUISE",decimate_CPU_SpeedCruise);
		m_nh_params.getParam("DECIM_CONTROLSIGNAL_SPEEDCRUISE",decimate_CONTROLSIGNAL_SpeedCruise);

		if (decimate_ADC_SpeedCruise>0 && decimate_CPU_SpeedCruise>0 && decimate_CONTROLSIGNAL_SpeedCruise>0 && decimate_ENCINC_SpeedCruise>0)
		{
			Decimation_config_SpeedCruise.decimate_ADC = decimate_ADC_SpeedCruise;
			Decimation_config_SpeedCruise.decimate_ENCINC = decimate_ENCINC_SpeedCruise;
			Decimation_config_SpeedCruise.decimate_CPU = decimate_CPU_SpeedCruise;
			Decimation_config_SpeedCruise.decimate_CONTROLSIGNAL = decimate_CONTROLSIGNAL_SpeedCruise;

			MRPT_LOG_INFO_FMT(" Speed Cruise Decimation: ADC=%i  ENCABS=%i  ENCINC=%i  CPU=%i  Control Signal=%i",decimate_ADC_SpeedCruise, decimate_ENCINC_SpeedCruise, decimate_CPU_SpeedCruise, decimate_CONTROLSIGNAL_SpeedCruise);
			this->CMD_Decimation_configuration(Decimation_config_SpeedCruise, m_serial_SpeedCruise);
		}
	}
	return true;
}

void VehicleControllerLowLevel::processIncommingFrame(const std::vector<uint8_t> &rxFrame, CSerialPort &m_serial) {
...
bool VehicleControllerLowLevel::iterate() {
	...
	if (m_mode_steer_changed) {
		...
		WriteBinaryFrame(reinterpret_cast<uint8_t *>(&cmd), sizeof(cmd),m_serial_Steer, m_serial_Steer_port_name, m_serial_Steer_port_baudrate);
		ROS_INFO_THROTTLE(1,"Sending new STEER controller mode: %s", m_mode_openloop_steer ? "MANUAL" : "AUTO");
	}
	if (m_mode_throttle_changed){
		...
		cmd.calc_and_update_checksum();
		WriteBinaryFrame(reinterpret_cast<uint8_t *>(&cmd), sizeof(cmd),m_serial_SpeedCruise, m_serial_SpeedCruise_port_name, m_serial_SpeedCruise_port_baudrate);
		ROS_INFO_THROTTLE(1,"Sending new SPEED CRUISE controller mode: %s", m_mode_openloop_throttle ? "MANUAL" : "AUTO");
	}
	// New joystick
	if (!m_autonomous_driving_mode && m_joy_changed) {
		...
		if (m_mode_openloop_steer) {
			...
			cmd.calc_and_update_checksum();
			WriteBinaryFrame(reinterpret_cast<uint8_t *>(&cmd), sizeof(cmd),m_serial_Steer, m_serial_Steer_port_name, m_serial_Steer_port_baudrate);
			ROS_INFO_THROTTLE(1, "Sending openloop STEER: %f", cmd.payload.SETPOINT_OPENLOOP_STEER_SPEED);
		} else {
			...
			cmd.calc_and_update_checksum();
			WriteBinaryFrame(reinterpret_cast<uint8_t *>(&cmd), sizeof(cmd),m_serial_Steer, m_serial_Steer_port_name, m_serial_Steer_port_baudrate);
			ROS_INFO_THROTTLE(1, "Sending closedloop STEER: %f", cmd.payload.SETPOINT_STEER_POS);
		}
		// Y: throttle
		if (m_mode_openloop_throttle) {
			...
			cmd.calc_and_update_checksum();
			WriteBinaryFrame(reinterpret_cast<uint8_t *>(&cmd), sizeof(cmd),m_serial_SpeedCruise, m_serial_Steer_port_name, m_serial_SpeedCruise_port_baudrate);
	...
	if (!nFrames && m_NOP_sent_counter++ > 20) {
		...
		TFrameCMD_NOP cmd;
		cmd.calc_and_update_checksum();
		return WriteBinaryFrame(reinterpret_cast<uint8_t *>(&cmd), sizeof(cmd),m_serial_Steer, m_serial_Steer_port_name, m_serial_Steer_port_baudrate);
	}
	// SPEED CRUISE
	nFrames = 0;
	...
	while (ReceiveFrameFromController(rxFrame_SpeedCruise, m_serial_SpeedCruise) && ++nFrames < MAX_FRAMES_PER_ITERATE) {
		// Process them:
	...
	if (!nFrames && m_NOP_sent_counter++ > 20) {
		...
		TFrameCMD_NOP cmd;
		cmd.calc_and_update_checksum();
	return WriteBinaryFrame(reinterpret_cast<uint8_t *>(&cmd), sizeof(cmd),m_serial_Steer, m_serial_SpeedCruise_port_name, m_serial_SpeedCruise_port_baudrate);
	}
	return true;
}
...
void VehicleControllerLowLevel::daqOnNewADCCallback(const TFrame_ADC_readings_payload_t &data, CSerialPort &m_serial) {
  ...
  if (&m_serial == &m_serial_Steer)
		m_pub_Steer_ADC.publish(msg);
	if (&m_serial == &m_serial_SpeedCruise)
		m_pub_SpeedCruise_ADC.publish(msg);
}
...
void VehicleControllerLowLevel::daqOnNewENCCallback(const TFrame_ENCODERS_readings_payload_t &data, CSerialPort &m_serial) {
	...
	if (&m_serial == &m_serial_Steer)
		m_pub_Steer_ENC.publish(msg);
	if (&m_serial == &m_serial_SpeedCruise)
 		m_pub_SpeedCruise_ENC.publish(msg);
}
...
bool VehicleControllerLowLevel::SendFrameAndWaitAnswer(const uint8_t *full_frame, const size_t full_frame_len, const int num_retries, const int retries_interval_ms, uint8_t expected_ans_opcode, CSerialPort &m_serial, std::string &m_serial_port_name, int &m_serial_port_baudrate) {
	...
	for (int iter = 0; iter < num_retries; iter++) {
		...
		// Send:
		if (!WriteBinaryFrame(full_frame, full_frame_len, m_serial, m_serial_port_name, m_serial_port_baudrate))
		...
		std::vector<uint8_t> rxFrame;
		// Cambiar m_serial
		if (this->ReceiveFrameFromController(rxFrame, m_serial) && rxFrame.size() > 4) {
...
bool VehicleControllerLowLevel::ReceiveFrameFromController(std::vector<uint8_t> &rxFrame, CSerialPort &m_serial) {
	rxFrame.clear();
...
bool VehicleControllerLowLevel::CMD_Decimation_configuration(const TFrameCMD_VERBOSITY_CONTROL_payload_t &Decimation_config, CSerialPort &m_serial){
	TFrameCMD_VERBOSITY_CONTROL cmd;
	cmd.payload = Decimation_config;
	cmd.calc_and_update_checksum();

	return SendFrameAndWaitAnswer(reinterpret_cast<uint8_t*>(&cmd), sizeof(cmd), m_serial);
}
